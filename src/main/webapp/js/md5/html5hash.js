		...
		.ready(
				function() {
					...
					function arrayBufferToWordArray(arrayBuffer) {
						...
						return CryptoJS.lib.WordArray.create(cp,
					...
					function selectFile(f) {
						(function() {
							...
							$("#showTable").append(
									'<tr><td colspan="2" class="red hash_file_info" id="'
											+ uid + '"></td>>/tr>');
							progressiveRead(
									f,
									function(data, pos, file) {
										...
										for (var j = 0; j < enabledAlgorithms.length; j++) {
											enabledAlgorithms[j].instance
													.update(wordArray);
										}

										if (doCRC32)
											crc32intermediate = crc32(
													new Uint8Array(data),
													crc32intermediate);

										// Update progress display
										var progress = Math
												.floor((pos / file.size) * 100);
									...
									function(file) {
										...
										if (doCRC32)
											results += '<tr><td class="green strong algoname">CRC-32</td><td>'
													+ decimalToHexString(crc32intermediate)
													+ '</td></tr>';

										for (var j = 0; j < enabledAlgorithms.length; j++) {
											results += '<tr><td class="green strong algoname">'
													+ enabledAlgorithms[j].name
													+ ' Hash</td><td class="algoresult">'
													+ enabledAlgorithms[j].instance
															.finalize()
													+ '</td></tr>';
										}
